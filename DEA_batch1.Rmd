...
keep10 <- rowAnys(cts > 10)
any_gt10 <- cts[keep10, ]

# check dim 
run("dim(any_gt10); # dim after")
```
...
condition <- factor(condition, level = c(control, treatments))
```
...
# draw
ggplot(
 data = df_all[df_all["control_cond"] == control, ],
 mapping = aes(
   x = log2FoldChange,
   y = -log10(padj),
   col = diffexpressed,
...
FDR = .05
FCT = 1.5
for (trtm in levels(condition)[levels(condition) != control]) {
  res <- results(
    dds,
    alpha = FDR,
    lfcThreshold = FCT,
